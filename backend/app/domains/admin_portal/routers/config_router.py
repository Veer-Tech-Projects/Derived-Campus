...
# [FIX] Import SessionLocal instead of get_sync_db
from app.database import SessionLocal 
from app.models import ExamConfiguration, DiscoveredArtifact, CollegeCandidate, College, SeatPolicyQuarantine

router = APIRouter(prefix="/config", tags=["Admin Portal: Configuration"])

# [FIX] Define the dependency locally
def get_sync_db():
    db = SessionLocal()
    try:
        yield db
...
def update_exam_mode(exam_code: str, req: ModeUpdateRequest, db: Session = Depends(get_sync_db)):
