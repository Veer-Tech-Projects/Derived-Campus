...
export default function AdminDashboard() {
  ...
  const handleModeChange = async () => {
    if (!targetExam || !targetMode || isUpdating) return;
  ...
  const renderStat = (value: number | undefined) => {
    if (value === undefined) {
      return <Loader2 className="w-6 h-6 animate-spin text-slate-300 mt-1" />;
    }
    return value;
  };
    ...
    <RoleGuard requiredRole="EDITOR">
      <div className="min-h-screen bg-[#FAFAFA] selection:bg-indigo-100 selection:text-indigo-900">
        ...
        <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-5">
              ...
              <Card className="h-full border-zinc-200/60 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden bg-white">
                ...
                <CardContent>
                  <div className="text-3xl font-bold text-zinc-900 tracking-tight h-9 flex items-center">
                    {renderStat(stats?.airlock_pending)}
                  </div>
            ...
            <Link href="/admin/triage" className="group">
              <Card className="h-full border-zinc-200/60 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden bg-white">
                ...
                <CardContent>
                  <div className="text-3xl font-bold text-zinc-900 tracking-tight h-9 flex items-center">
                    {renderStat(stats?.triage_pending)}
                  </div>
            ...
            <Link href="/admin/triage/seat-policy" className="group">
              <Card className="h-full border-zinc-200/60 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden bg-white">
                ...
                <CardContent>
                  <div className="text-3xl font-bold text-zinc-900 tracking-tight h-9 flex items-center">
                    {renderStat(stats?.seat_policy_pending)}
                  </div>
            ...
            <Link href="/admin/media-governance" className="group">
              <Card className="h-full border-zinc-200/60 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden bg-white">
                <div className="absolute top-0 right-0 p-4 opacity-[0.03] group-hover:opacity-10 transition-opacity">
                  <Image className="w-24 h-24 text-sky-500 rotate-12" />
                </div>
                <CardHeader className="pb-2">
                  <div className="w-10 h-10 rounded-lg bg-sky-50 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform duration-300 border border-sky-100">
                    <Image className="w-5 h-5 text-sky-600" />
                  </div>
                  <CardTitle className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Media Governance</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="text-3xl font-bold text-zinc-900 tracking-tight h-9 flex items-center">
                    {renderStat(stats?.media_pending)}
                  </div>
                  <div className="flex items-center mt-2 text-xs font-semibold text-sky-600 group-hover:translate-x-1 transition-transform">
                    Triage Candidates <ArrowRight className="w-3 h-3 ml-1" />
                  </div>
                </CardContent>
              </Card>
            </Link>
          ...
          {/* Confirmation Modal */}
          <Dialog open={!!targetExam} onOpenChange={(open) => {
            if (!open && !isUpdating) setTargetExam(null)
          }}>
